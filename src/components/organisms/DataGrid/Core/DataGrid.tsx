...
import { DefaultRowRenderer } from "../Extensions/RowRenderers/DefaultRowRenderer";
import { DefaultGroupRowRenderer } from "./GroupRow";
import SummaryRow from "./SummaryRow";
import EditCell from "./EditCell";
import DragHandle from "./DragHandle";
import { default as defaultSortStatus } from "./SortStatus";
import { CheckboxFormatter as DefaultCheckboxFormatter } from "../Extensions/Formatters";
import {
...
export interface DataGridProps<R, SR = unknown, K extends Key = Key>
  ...
  onRowClick?: Maybe<
    (row: R, column: CalculatedColumn<R, SR>, e?: any) => void
  >;
...
function DataGrid<R, SR, K extends Key>(
  ...
  const rawColumns = useMemo(() => {
    let autoColumns = [..._rawColumns];

    const noHideColumns = autoColumns.filter((e) => !e.hide);

    const gridWidthSum = _.sumBy(noHideColumns, "width");
    ...
    switch (autoSize) {
      case "auto-fill":
        ...
        */
        const noDefineColumns = autoColumns.filter(
          (e) => !e.width && !e.minWidth && !e.hide
        );
        const remainerWidth = gridWidth - gridWidthSum;

        if (noDefineColumns.length && remainerWidth > 0) {
          noDefineColumns.forEach((column) => {
            const newColumn = {
              ...column,
              width: remainerWidth / noDefineColumns.length,
            };

            const index = autoColumns.indexOf(column);
      ...
      case "auto-fit":
        ...
        */
        ...
        if (gridWidthSum < gridWidth) {
          const noMaxWidthColumns = noHideColumns.filter((e) => !e.maxWidth);

          const remainer =
            (gridWidth - gridWidthSum) / noMaxWidthColumns.length;

          noMaxWidthColumns.forEach((column) => {
            const newColumn = {
              ...column,
              width: (column.width as number) + remainer,
            };
  ...
  const rowHeight = rawRowHeight ?? 36;
  const headerRowHeight =
    rawHeaderRowHeight ?? (typeof rowHeight === "number" ? rowHeight : 48);
  const summaryRowHeight =
    rawSummaryRowHeight ?? (typeof rowHeight === "number" ? rowHeight : 35);
  const rowRenderer =
    renderers?.rowRenderer ??
    defaultComponents?.rowRenderer ??
    DefaultRowRenderer;

  const groupRowRenderer =
    renderers?.groupRowRenderer ??
    defaultComponents?.groupRowRenderer ??
  ...
  const { allRowsSelected, indeterminate } = useMemo((): {
    ...
    return {
      allRowsSelected:
        ...
        rawRows.every((row) => selectedRows.includes(rowKeyGetter(row))),
      indeterminate:
        length !== 0 &&
        selectedRows != null &&
        rowKeyGetter != null &&
    ...
    for (
      ...
      if (isGroupRow(row)) {
        ...
        const isGroupRowSelected =
          isSelectable &&
          row.childRows.every((cr) => selectedRows.includes(rowKeyGetter!(cr)));

        rowElements.push(
          groupRowRenderer(row.id, {
      ...
      if (typeof rowKeyGetter === "function") {
        ...
        isRowSelected = selectedRows?.includes(key) ?? false;
      } else {
        ...
        <DataGridDefaultComponentsProvider value={defaultGridComponents}>
          ...
          {rows.length === 0 && noRowsFallback ? (
            ...
            <>
              ...
              <RowClickProvider value={onRowClickLatest as any}>
