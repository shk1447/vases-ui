...
interface ViewportRowsArgs<R, K> {
  rawRows: readonly R[];
...
export function useViewportRows<R, K>({
  rawRows,
  rowHeight,
  clientHeight,
  scrollTop,
  ...
  const [rows, isGroupRow] = useMemo((): [
    ...
    let flattenedRows: Array<R | GroupRow<R>> = [];
    const expandGroup = (
      ...
      Object.keys(rows).forEach((groupKey, posInSet, keys) => {
        ...
        let selectedChildRows: any = [];
        if (rowKeyGetter != null) {
          selectedChildRows = childRows.filter((row) =>
            selectedRows?.includes(rowKeyGetter(row))
          );
        }
  ...
  }, [expandedGroupIds, groupedRows, rawRows, selectedRows, rowKeyGetter]);
