...
export default class SelectionController extends BaseAbsctractController {
  ...
  mousedownFunction(e: KonvaEventObject<MouseEvent>): void {
    ...
    if (
      ...
      const x1 = this._stageWrapper?.stage?.getPointerPosition()?.x;
      const y1 = this._stageWrapper?.stage?.getPointerPosition()?.y;
      if (x1 && y1) {
      ...
      if (e.target.getAttr('selected')) {
        ...
        if (
          ...
          const filtered = selected.filter(
            (node: Konva.Node) => node.id() !== e.target.id(),
          );
          if (filtered.length) {
  ...
  mousemoveFunction(e: KonvaEventObject<MouseEvent>): void {
    ...
    if (this._hasMousedown && e.evt.buttons === 1) {
      const x2 = this._stageWrapper?.stage?.getPointerPosition()?.x;
      const y2 = this._stageWrapper?.stage?.getPointerPosition()?.y;
  ...
  mouseupFunction(e?: KonvaEventObject<MouseEvent>): void {
    ...
    const box = this.selectionRect?.getClientRect({ skipStroke: true });
    const selected = this._stageWrapper?.currentLayer?.getChildren(
      (shape: Konva.Node) => {
        ...
        return Konva.Util.haveIntersection(
          box,
          shape.getClientRect({ skipStroke: true }),
        );
      },
    );
